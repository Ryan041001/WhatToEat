...
public class RestaurantReviewQueryApplicationService {
    ...
    private ReviewSummary toReviewSummary(RestaurantMetricSnapshotEntity snapshot) {
        boolean aiReady = "ready".equalsIgnoreCase(snapshot.getAiStatus());
        List<String> aiTags = aiReady
                ? Stream.of(snapshot.getAiTag1(), snapshot.getAiTag2())
                .filter(tag -> tag != null && !tag.isBlank())
                .toList()
                : List.of();
        String aiSummary = aiReady ? snapshot.getAiSummary() : null;
        List<String> recommendedScenarios = aiReady
