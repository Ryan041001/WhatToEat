...
1. **登录页已接后端登录接口**
   ...
   - 登录失败时现在直接暴露真实错误，不再自动切本地 mock 会话。
...
2. `choice-history / recommendation-feedback` 的历史查询页展示（GET 已有接口，但前端还未做独立历史页）
...
1. 前端正式链路只调用 `POST /api/v1/recommendations/ask/stream`
2. 页面必须按事件增量渲染回答，不等待整段文本返回后一次性展示
3. 同步 `POST /api/v1/recommendations/ask` 仅保留为兼容接口，不进入小程序正式推荐入口
...
- 筛选仍依赖全局缓存数据，但排序切换已重新请求后端真实排序接口
- 黑名单切换仍是本地状态切换
...
- `app.toggleBlacklist()` 已调用后端黑名单接口
- 本地缓存只保存服务端返回后的同步结果
...
- 列表页、详情页和 AI 推荐现在以同一份服务端黑名单为准
- 如果状态不一致，应优先排查登录态或黑名单接口本身，而不是前端本地缓存

### 8.2 登录失败时不再走 mock fallback

当前 `frontend/pages/index/index.js` 在登录失败时会直接提示真实错误。
...
- 联调时优先确认后端服务可达，再进行评论 / 黑名单 / AI 个性化链路验证
- 如果出现 `ERR_CONNECTION_REFUSED`，先检查当前 API Base URL、宿主机端口监听状态，以及真机是否仍在请求 `127.0.0.1`
...
1. 首次进入 AI 页时，如果没有历史会话，会自动发起一条默认流式推荐
2. 如果有上一轮会话缓存，则优先恢复缓存，不重复发首轮请求
3. 从首页进入时仍可通过预热问题覆盖默认首问
推荐的首轮体验方式：

1. 小程序启动后先拿定位和餐厅列表
2. 用户进入对话页时自动发起一条默认流式请求
3. 若已有上一轮会话，则优先恢复文本和卡片，再由用户决定是否继续追问
