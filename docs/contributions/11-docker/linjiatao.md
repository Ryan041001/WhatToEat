...
- [x] 前端容器提供浏览器可访问的 Docker 预览页，微信小程序源码挂载在 `/miniprogram/`
...
- [x] 生产环境 `compose.prod.yaml` 前端服务（团队分支已完成）

### 3. 自动化部署

- 选项 A：Docker 镜像工作流已覆盖 frontend / backend / ai-service。
- 选项 B：团队已提供 `deploy.sh`，用于生产编排一键启动。
...
3. 问题：直接把微信小程序源码挂到 nginx 根目录时，浏览器访问 `/` 会返回 403，健康检查也无法稳定通过。
   解决：容器根路径提供 Docker 预览页，源码目录统一挂载到 `/miniprogram/`。
